...
1. **Read the references now** (they ship with this skill — read via the `Read` tool; if `${CLAUDE_SKILL_DIR}` doesn't expand, they're in the `references/` folder next to this SKILL.md):
   ...
   - 🤝 **Если файл — Shared (`LxMK9OD6jvgPWPVg25EZAM`), читай `${CLAUDE_SKILL_DIR}/references/shared-delta.md`** — переопределения S1–S8: дока обслуживает ОБЕ платформы, секция «Поведение по платформам» обязательна, обязательный разбор ширины (`maxWidth` нигде не задан), нейтральный стенд. `desktop-delta` там НЕ применяется.

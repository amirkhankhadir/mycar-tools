...
- **Шесть публичных компонентов:** `filters-panel` COMPONENT `8800:53651` · `filter-button` SET `8792:50407` · `filter-menu/default` COMPONENT `8792:46839` · `/no-results` `8810:54011` · `/loading` `8810:54012` · **`filter-menu/date` SET `8866:70753`**. ⚠️ Меню переименованы: было `filter-dropdown-menu/*`. Старой доки на странице не было — переносить было нечего.
...
- **verified 28.08.2026 (вторая волна правок):** у одиночного выбора счётчика больше НЕТ — применённая кнопка показывает само значение через TEXT-свойство `selected-value#8895:351` (def «Option», рисуется как «Подпись: значение»). У этого текста `maxWidth 180` + `textTruncation='ENDING'` + `maxLines 1` → длинное значение обрезается многоточием; у `label` обрезки нет вовсе, поэтому вписанное руками значение раздувает кнопку без предела. Счётчик-бейдж остался только у мульти-выбора, у быстрого фильтра нет ни того ни другого. Период «03.08.2026 – 13.08.2026» занимает 167 из 180 — влезает впритык.
- **`filter-menu/date` SET `8866:70753`:** оси `range-picker` × `filled`, 4 варианта, 300×408 (пусто) / 300×460 (заполнено). Кнопка «Очистить фильтр» живёт ВНУТРИ `filled=true`, своего `show-button` у меню нет. Поле — вложенный exposed инстанс `date-picker-input` со своей осью `filled`.
- **`filter-menu/default`: дефолт `show-button` переведён в `false`** (был `true` — и показывал кнопку очистки в меню, где ничего не выбрано). В вариантах чипа `state=selected, is-applied=true` теперь стоит ЯВНЫЙ `true`.
- ⛔ **Механика связки, проверена экспериментом:** оверрайд свойства при instance-swap переезжает **по имени свойства** и только если это НАСТОЯЩИЙ оверрайд (значение ≠ дефолта компонента-источника); совпадение с дефолтом не переносится. **Variant-значения при свапе не переносятся вообще** — инстанс встаёт в тот вариант, на который свапнули. Отсюда: связать ось `filled` меню с `is-applied` кнопки оверрайдами нельзя, только булево свойство ездит.
- **Схема на переменной (сделана дизайнером):** локальная коллекция `filter-state`, скрыта от публикации, моды `not-applied` / `applied` + строковая `is-applied` («false»/«true»). Моды проставлены на всех четырёх вариантах чипа `state=selected`. В макете дизайнер биндит `filled` у меню и `is-applied` у кнопки к переменной — тогда один переключатель меняет и кнопку, и содержимое календаря. Скриптом это делается через `figma.variables.createVariableAlias(v)` → `inst.setProperties({'prop': alias})`. ⚠️ После `setProperties` на variant-свойстве дети инстанса протухают — меню перечитывать заново.
- **Правило дизайнера про год (28.08.2026):** в фильтре дату пишут полностью, `ДД.ММ.ГГГГ`, год показывают ВСЕГДА — даже текущий. Это осознанное исключение из гайда форматов Mўlo (там год ставят только для не-текущего): дату вводят по маске в поле, и маска требует год. Разделитель диапазона и сокращения месяцев берут из гайда как обычно.
- **Дока обновлена 28.08.2026:** +секция «Меню даты» (два меню, карточка «Как выглядит кнопка с выбранной датой», пилюля про привязку к переменной), +карточка «Как отличить тип по применённой кнопке» в «Типах фильтров», +строки `filter-menu/date` в «Семействе» и `date-picker-input` в «Соседних компонентах»; переписаны «Контент», анатомия и обе пары Do & Don't под `selected-value`.

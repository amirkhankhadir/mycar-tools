...
**Секция «РАЗМЕРЫ» у компонента из Shared — два уровня.** Сначала card с полной лестницей
(превью + по заметке на ступень: что это за размер и где стоит). Под ней — `row-платформы`:
HORIZONTAL gap `space-4`, две обычные нейтральные карточки на `FILL`, в каждой три текста —
заголовок платформы (`body/sm-medium`, primary), ряд ступеней через `·` (`body/lg-medium`,
primary) и объяснение, каких ступеней платформа НЕ берёт и чем их заменяет (`body/sm-regular`,
secondary). Нужно там, где один компонент обслуживает обе платформы, а набор ступеней у них
разный — иначе разработчик решит, что должен реализовать все. Эталон: Buttons `4:3274`,
Icon Buttons `4:6471`.
...
### payment-success-page — COMPONENT `16248:4912` на странице `📱 templates/payments` (`4548:1123`) · дока **finalized `16257:913`**, старой доки не было
- **Не сет, а один COMPONENT**, 393×852, v1.1.0. Props: `title#16248:1` TEXT (def «Готово!») · `subtitle#16248:2` TEXT (def «Ваш платеж принят») · `show-receipt-button#16248:0` BOOL (def true). Больше наружу ничего не выведено.
- структура: `top-nav` (style=screen, show-title=false, right-content=close) поверх `success-block` (fill `bg-surface/neutral/base-container`, нижние радиусы `corner-radius/lg`, clipsContent) → `video-container` + `text-and-button-container` (title `title/lg-medium` primary, subtitle `body/lg-regular` secondary, `receipt-button` = on-container-button 44 с иконкой `main-bunch/receipt`); внизу `bottom-action-bar` (bg=transparent, 1-button, primary-button 56 «Вернуться в платежи»).
- **Реакций прототипа нет ни на одном узле.** Подписи обеих кнопок и `type` ролика пропами шаблона НЕ выведены — правятся во вложенных инстансах.
- **mobile-only** — проверено фактом: в Desktop Library страницы templates/payments нет, блок-указатель не нужен.
- **`.payment-success-page/animated-video`** — приватный (Скрыт) SET `16248:2512`, 480×480, внутри RECTANGLE `video` 420×420 с VIDEO-заливкой (`scaleMode FILL`, `scalingFactor 0.5`); исходники вертикальные 1080×1920. Оси: `type` [purchase 5,0 с · other 3,9 с] × `theme` [light · dark]. Ноды: dark/purchase `16248:2511` · dark/other `16248:2531` · light/purchase `16248:2510` · light/other `16248:2530`.
- ⭐ **`theme` инстанса привязана к ПЕРЕМЕННОЙ** `theme` (STRING, key `156b24e2db494ba0e3fa0dc874428e58a8d89d48`, коллекция `brand-theme-semantics`, light в светлых модах / dark в тёмных) → тему ролика вручную не переключают, она едет за темой макета. Такую привязку видно только в `boundVariables.componentProperties` инстанса — обычный обход `boundVariables` её пропускает, потому что она вложена.
- у приватного набора в `documentationLinks` лежит **не дока, а архив с исходниками** роликов (решение дизайнера); ссылка на доку стоит у публичного компонента.

### something-went-wrong-page — COMPONENT_SET `16454:12431` на странице `📱 templates/system-status` (`16454:3518`) · дока **finalized `16460:804`**, старой доки не было
- **Сет из двух вариантов**, 393×852, v1.0.0 «Готов к разработке». Единственное свойство — `layout` VARIANT [page `16454:11664` (def) · large-modal `16454:12432`]. Тексты и подпись кнопки наружу НЕ выведены — правятся оверрайдом в инстансе.
- структура варианта: `top-nav` (page → `style=screen` с `left-content action-type=back`; large-modal → `style=large-modal` с крестиком) + `text-container` (TEXT `title` «Что-то пошло не так» + TEXT `subtitle` «Мы уже разбираемся с проблемой. Попробуйте обновить страницу или вернуться позже») + инстанс `.something-went-wrong-image` + `bottom-action-bar` (`bg=transparent`, `adaptive=false`) → `buttons` `1-button` → `on-container-button` 56 «Обновить».
- **`.something-went-wrong-image`** — приватный SET `16454:11397`, ось `theme` [light `16454:11396` / dark]. У инстанса внутри шаблона `theme` **привязана к переменной** `theme` (key `156b24e2db494ba0e3fa0dc874428e58a8d89d48`) — тот же приём, что у ролика в `payment-success-page`: вручную тему не переключают.
- **mobile-only** — проверено фактом: в Desktop Library нет ни templates-страниц, ни системных экранов.
- соседи для раздела «когда НЕ использовать» (проверены по файлам): `empty-state` / `status-state` / `loading-state` — Shared `🧩 page-states` (`13:17137`); `inline-alert` / `banner-alert` — Shared `🧩 alerts`; тосты — Mobile `🧩 toast-notifications`; плановые техработы — Mobile `⚙️ templates/maintenance-page (WIP)`. В Mobile `🧩 page-states` (`642:13196`) лежит только `feedback-state` — не перепутать со Shared-страницей того же имени.
- ⛔ **Кадр-обрезка экрана шириной меньше 393 режет именно то, ради чего строится.** Кроп 288 px по центру (`x=-52`) съел и стрелку назад, и крестик — разница вариантов исчезла, карточки читались как два пустых белых поля. Для шапок берём полную ширину 393 в широкой карточке (`card` HORIZONTAL: стенд FILL ≈436 + текстовая колонка FIXED 240), а не два кропа в ряд.

### slider — COMPONENT `16287:15147` на странице `🧩 sliders` (`1048:637`) · дока **ЧАСТЬ 3 семейного фрейма `15672:33529`**, старой доки не было
- **Не сет, а один COMPONENT**, 457×86 (ширина FIXED, высота HUG), v1.0.0 «Готов к разработке». Наружу выведены ТОЛЬКО два булевых свойства: `show-heading#16287:3` (def true) · `show-input#16287:4` (def true). Текст заголовка и значение поля — во вложенных инстансах (`heading` → `title#2793:37`, `text-input` → `value#3657:109`).
- структура: VERTICAL → `heading` (инстанс Shared-набора `heading`, `size=sm`) + `Frame 3` (HORIZONTAL) → `slider-container` (FILL, высота `sizing/size-10`) → **`change-right-paddings-here`** (трек, высота `sizing/size-3`, радиус `corner-radius/sm`, фон `bg-surface/neutral/base-accent`) → `range-container` (gap **−28**) → `range-bg` (активная зона, `bg-surface/brand/strong`) + `bullet`; справа `text-input` `size=48, filled=true`, ширина `sizing/size-16` (64), `minWidth` `sizing/size-10`.
- **позиция ползунка = правый паддинг `change-right-paddings-here`** (у range-slider — левый И правый у `change-paddings-here`). Ползунок вручную не двигают.
- **`.range-slider/bullet` SET `15598:904`** (общий с range-slider): `is-pressed` [false `15598:905` / true `15598:909`] · `show-tooltip#4218:1` BOOL (def false). ⚠️ **Тултип существует только в варианте `is-pressed=true`** и является ПРЯМЫМ ребёнком bullet (не внутри `tap-target-size`). Дефолтный текст тултипа — «0.0», правится как обычный TEXT. `tap-target-size` 40×40 вокруг кружка 28.
- **verified поведение:** ON_DRAG на ползунке; у трека нет ни disabled, ни ошибки, ни фокуса. **Стопперов нет** (в отличие от range-slider с `14-px-stopper`): на минимуме ползунок встаёт на −27 px от начала трека (вылезает влево почти целиком), на максимуме — ровно во внутренний край. Асимметрия из-за `gap=-28`; дизайнеру озвучено.
- ⚠️ **Поле 64 px вмещает 2–3 знака.** У `text-value` ширина ограничена родителем, поэтому `maxLines=1 + ENDING` РАБОТАЕТ: «1 500 000» рендерится как «1 5…» (проверено). Отсюда правило доки: под крупные значения либо расширяют поле (и оно забирает часть шкалы), либо выключают поле и показывают тултип.
- **ПРАВИЛО ДИЗАЙНЕРА (главный раздел доки):** поле есть → тултип НЕ показываем; поля нет → тултип обязателен. Значение всегда должно быть видно числом.
- mobile-only: в Desktop Library страниц со слайдерами нет (проверено по `figma.root.children`).

### swipe-card — COMPONENT_SET `4545:1169` на странице `🧩 swipe-actions` (`4544:9945`) · дока **finalized `15672:33619`**, старой доки не было
- props: `card-slot#6997:43` SLOT · `card-corners` [rounded (def) | sharp] · `swipe` [none (def) | left | right] · `#-of-actions` [none (def) | 1 | 2 | 3] — 14 вариантов. `description` непустое (`[Mobile] Темплейт для карточек…`), `documentationLinks` стоит.
- приватные: `.swipe-actions/core-item` SET `4545:752` — `status` [danger `4545:751` · primary `4545:749` · success `4545:750` · neutral `5228:2598`] · `action-label#5253:24` TEXT · `icon#4545:0` INSTANCE_SWAP; блоки `.swipe-actions-left|right/{1,2,3}` шириной 118 / 212 / 306.
- значения свопа иконок, которыми набраны превью доки: галочка `4497:1306` (`selection/Checkmark`), корзина `4726:39815` (`general/Delete`).
- радиус `rounded` = **20** на всей карточке; у блока действий своего радиуса нет — он обрезается контуром карточки.
- **направление:** `swipe=left` — действия выезжают справа, `swipe=right` — слева. Промежуточных положений в наборе нет: только закрытое и полностью раскрытое.
- 03.09.2026 добавлена секция **«ПОЛНЫЙ СВАЙП»** (`16320:1044`) + правки в «ПОВЕДЕНИЕ» и «ДОСТУПНОСТЬ» и четвёртая пара Do & Don't. **Правило дизайнера:** полный свайп на удаление допустим ТОЛЬКО если после срабатывания спрашивают подтверждение; пороги и тайминги в доке — лучшие практики, дизайнер разрешил их назначить.
...
### Новый компонент в уже задокументированном семействе (slider, 02.09.2026)

- ⭐ **Имя безымянного компонента ищи на бейдже `.component-version` рядом с ним.** `slider` лежал на канвасе как `Frame 2`, а на бейдже в 100 px выше уже стояли «Component Name: slider», версия 1.0.0 и статус «Готов к разработке». Придумывать имя с нуля не надо — сначала прочитай бейдж (и сверь с соседями по семейству), потом предлагай.
- ⛔ **Добавляя часть в семейную доку, перечитай её шапку и вводки других частей.** Появление `slider` сделало ложью две фразы уже опубликованной доки: «На странице живут два независимых компонента…» и «Одиночного ползунка на одно значение в библиотеке нет». Такие фразы — не стилистика, а прямая дезинформация, и правятся в том же проходе, что и новая часть. Ищи их по словам «нет», «только», «два/три», «не существует».
- ⛔ **Плоский `findAll()` не показывает вложенность.** Дамп bullet вернул подряд `tap-target-size` и `tooltip`, я построил путь `bullet → tap-target-size → tooltip` — и получил «miss tooltip». Тултип оказался ПРЯМЫМ ребёнком bullet. Либо возвращай `n.parent.name` вместе с узлом, либо ищи `findOne(n=>n.name==='…')` от корня, а не собирай путь по порядку строк.
- **Инстанс шире колонки — уменьшай инстанс, а не колонку.** Компонент 457 px в ряду из двух карточек (по ~320 внутри) клипается; `inst.resize(296, inst.height)` безопасен, когда трек на FILL, а вложенное поле фиксированной ширины (64) в габарит влезает. FILL на инстансе по-прежнему нельзя (рант-эвей).
- **Пилюля/заметка после `appendChild` требует `layoutSizingHorizontal='FILL'` отдельно** — иначе она хагается по тексту и выезжает за карточку. Поймано дважды за одну сборку: FILL внутри хелпера ставится только внутреннему тексту, сама пилюля остаётся HUG.

### Растянуть вложенное действие внутри инстанса нельзя (swipe-card, 03.09.2026)

Нужно было показать состояние «предел полного свайпа» — действие во всю ширину строки. В наборе такого варианта нет, и переделать инстанс не вышло:

- ⛔ **`resize()` на инстансе, вложенном в другой инстанс, молча не срабатывает.** `swipe-action-1` внутри `swipe-card`: `layoutSizingHorizontal='FIXED'` проставляется и читается обратно, а `resize(345,…)` оставляет прежние 118 — **без ошибки**. Тот же компонент, созданный отдельно (`comp.createInstance()` от `.swipe-actions/core-item`), резайзится свободно: 94×127 → 345×51.
- ⛔ **`detachInstance()` на ВЛОЖЕННОМ инстансе неявно детачит родителя и меняет его id** — следующий `getNodeByIdAsync(родитель)` падает «does not exist», и весь скрипт откатывается (атомарность спасла файл).
- ⛔ **После `setProperties` на вложенном инстансе дальнейшие мутации того же поддерева в ТОМ ЖЕ вызове игнорируются молча** (не падают). Сначала это выглядит как «resize не работает» — на деле handle протух. Разносить по разным вызовам `use_figma`.
- ⭐ **Рецепт «состояния, которого нет в наборе»:** FRAME `layoutMode:'NONE'`, `clipsContent=true`, радиусы как у карточки → внутрь клон живого инстанса строки со сдвигом (`x = ширина действия`) + отдельный инстанс действия нужной ширины на `x=0`. Живыми остаются оба компонента, детач не нужен.
  - у сдвинутой строки **обнулить левые радиусы** (`topLeftRadius`/`bottomLeftRadius = 0`), иначе между действием и строкой появляется белая линза;
  - после любых правок **перечитывать `x`/`y` клона**: на нашем превью координата сама съехала с 345 на 304 и её пришлось выставлять заново.
- Для пары Do & Don't такой макет ужимается до 300 px (действие 255) — в колонку 360 с внутренними 320 влезает; 393 не влезает.

### Видео в компоненте: чем оно НЕ документируется и как его опознать (payment-success-page, 02.09.2026)

- ⛔ **VIDEO-заливка рендерится первым кадром.** И `get_screenshot`, и `node.screenshot()` отдают статичный первый кадр — у наших роликов это сплошная белая или чёрная плашка. По скриншоту нельзя ни увидеть сюжет, ни сравнить два ролика, ни проверить, что вариант вообще не пустой. Следствие для доки: превью анимации собирают из **кадров, вытащенных из исходников**, а в анатомии честно пишут строкой, что в статике зона пустая.
- ⭐ **`videoHash` варианта = SHA-1 файла-исходника, байт в байт.** Это единственный надёжный способ сопоставить вариант с файлом: `shasum -a 1 *.mp4` и сверить с `fills[0].videoHash`. Проверено на 4 роликах — совпали все. Имена слоёв и порядковые номера в именах файлов **врут** (у нас `-1-` оказался `other`, а `-2-` — `purchase`, при слоях `video-light`/`02_2 2`); догадки по ним стоили неверной доки, которую поймал дизайнер.
- Косвенный способ, когда файлов нет: первые кадры роликов различаются на пару единиц яркости — уменьшить до 8×8 и сравнить сигнатуры. Работает, но слабее хеша.
- **Кадр из видео вытаскивается без ffmpeg** — коротким swift-скриптом на `AVAssetImageGenerator` (`swiftc` есть в системе). Длительность и размер читаются `mdls -name kMDItemDurationSeconds -name kMDItemPixelWidth`.
- **Картинку кладут в заранее созданный прямоугольник** через `upload_assets` с `nodeIds` (по одному id на каждый upload URL, порядок совпадает) → `POST` байтов на `submitUrl` → заливка проставляется сама. Плитка под вертикальный кадр 1080×1920: 154×274, четыре в ряд влезают в 736.

### Обрезанный инстанс — как показать кусок длинного экрана

Для Do & Don't по экрану-шаблону не нужен весь экран на 852 px: FRAME с `clipsContent=true` нужного размера, внутри инстанс со сдвигом `x`/`y` в минус — видно ровно нужный блок. У `payment-success-page` текстовый блок = кадр 300×96 при `x=-47, y=-486`, блок с кнопкой счёта = 300×150. Инстанс при этом живой, свойства ставятся как обычно.

⛔ **Пара Do/Don't обязана различаться ВИЗУАЛЬНО.** Первая версия пары «кнопка счёта» показывала два одинаковых кадра, а разница («счёта в сценарии нет») жила только в подписи — карточка ничего не объясняла. Если различие невидимо, меняй пример, а не подпись: у нас сработала подмена подписи кнопки («Электронный счет» против «Открыть») через вложенный инстанс.
...
### Слоты, свойства и чужие дефекты (filters, 28.08.2026)

- ⛔ **У SLOT-узла нет свойств `minChildren` / `allowPreferredValuesOnly`** — чтение `slot.minChildren` падает с «no such property». Настройки слота живут у КОМПОНЕНТА: `component.componentPropertyDefinitions['<slot-prop>'].slotSettings` (+ `preferredValues` рядом). Читать только оттуда.
- **Добавить TEXT-свойство целому набору можно одним вызовом:** `set.addComponentProperty('label','TEXT','Filter')` → вернёт `label#<id>`, затем в каждом варианте `t.componentPropertyReferences = {characters: propId}`. Переименование оси — `set.editComponentProperty('старое', {name:'новое'})`, имена вариантов и оверрайды переезжают сами. Проверено на 22 вариантах filter-button; после такой правки библиотеку надо переопубликовать.
- ⛔ **`tooltip` из Shared при `type=plain` показывает `supporting-text`, а НЕ `title`.** Ставить текст в `title#3641:22` бесполезно — на канвасе останется «Tooltip text». Ключ базового варианта (`style=base, type=plain, width=auto, position=▲ top-center`) — `01f4573b570f6888d16810dd8ed37b28a547217d`; `show-title#5027:18` выключать.
- ⛔ **`dropdown-menu-items/base`: чекбокс внутри `selector` НЕ привязан к `is-selected`.** У выбранной строки мульти-выбора подсвечивается фон и включается слой `checkmark`, а вложенный `checkbox` остаётся `is-selected=false` → в превью пустой квадрат. Лечится вручную: `it.findOne(n=>n.name==='selector').children.find(n=>n.name==='checkbox').setProperties({'is-selected':'true'})`. Дефект чужого (уже опубликованного) семейства — флагать дизайнеру, компонент не трогать.
...
- ⛔ **Абсолютные id детей слота протухают после КАЖДОЙ правки** — рецепт «собери `I<инст>;<слой>;<ребёнок>` списком и правь по нему» работает только на первую строку, дальше `getNodeByIdAsync` отдаёт `null`. Надёжный цикл: в каждом вызове заново спускаться `кнопка → меню → list-container → slot`, читать текущие `title` и править ПЕРВУЮ, где ещё стоит плейсхолдер. Один вызов — одна строка.
- ⛔ **Массовый `findAll` по наличию свойства цепляет лишнее.** Правка «всех инстансов, у которых есть `selected-value`» переписала и неприменённые кнопки в первой паре Do & Don't: свойство объявлено на всём наборе, а рисуется только при `is-applied=true`. Фильтровать по ЗНАЧЕНИЮ варианта (`componentProperties['is-applied'].value`), а не по наличию свойства, и после пакетной правки перечитывать все затронутые подписи.
- ⛔ **RU-прогон: `NB` собирать ТОЛЬКО через `String.fromCharCode(160)`** (и класс пробелов в регулярке — так же). Литерал в исходнике скрипта приезжает обычным пробелом, и прогон тогда не добавляет неразрывные, а СТИРАЕТ уже расставленные: 28.08.2026 на доке filters так осыпались 114 узлов, в файле осталось 2 nbsp вместо 378. После прогона обязательна проверка: суммарное число символов с кодом 160 по фрейму должно вырасти, а не упасть.

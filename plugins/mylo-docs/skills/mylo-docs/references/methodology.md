...
## ⛔ Вопросы дизайнеру — НИКОГДА не в документации (правило дизайнера, 20.08.2026)

Дока — это ответы, а не переписка. Читатель приходит узнать, как пользоваться компонентом, и любой «Вопрос дизайнеру: так задумано или это недосмотр?» для него мусор: он не знает ответа и не должен его искать.

Куда девать сомнения:
- **все вопросы, догадки и «нужно подтвердить» → в ОТЧЁТ по задаче** (то есть в чат), отдельным разделом;
- в доку попадает только то, что ты знаешь. Если факт не подтверждён — либо опиши наблюдаемое поведение нейтрально, без оценки, либо не пиши вовсе.

Запрещённые формулировки в теле доки: «Вопрос дизайнеру», «нужно решение дизайнера», «подтвердите», «так задумано или недосмотр», «уточните у», «стоит ли», «нужен ответ продукта», а также целые секции вида «ВОПРОСЫ ДИЗАЙНЕРУ» / «ОТКРЫТЫЕ ВОПРОСЫ».

Как писать вместо этого:
- ❌ «Вопрос дизайнеру: на xs заголовок и подзаголовок стоят вплотную. Так задумано?»
- ✅ «На xs отбивки между заголовком и подзаголовком нет.» — факт, без оценки. А сомнение уходит в отчёт.

Если поведение выглядит как пробел набора, допустимо написать нейтрально: «варианта X в наборе нет» — это факт, а не вопрос.

### Чего не хватает — в бэклог, а не в доку

Отдельный запрет: **не писать в доке, что решение ещё не принято.** Формулировки «договорённости пока нет», «правило нужно определить», «зафиксируйте до первого экрана», «пока не решено» в теле доки недопустимы — читателю они не помогают, а доку превращают в протокол незакрытых вопросов.

Такие пробелы идут в бэклог проекта (файл памяти `project_mylo_library_split.md`, раздел бэклога) и в отчёт. В доке остаётся только то, что решено.

- ❌ «Для десктопа договорённости о месте пока нет: угол окна и отступы решает макет. Зафиксируйте их до первого десктопного экрана.»
- ✅ «На мобилке тост выравнивают по нижнему краю статус-бара, в пределах безопасной области.» — а отсутствие десктопного правила уходит в бэклог.

## ⛔ Старую документацию НЕ выбрасывать (правило дизайнера, 20.08.2026)

Пересобирая доку в новом хаус-стайле, ты обязан **отразить в новой всё содержательное из старой**. Старая дока — накопленные решения команды, а не черновик, который можно заменить своим текстом.

Порядок работы:
1. Прочитай старую доку ЦЕЛИКОМ и выпиши все её утверждения списком.
2. По каждому реши: перенести как есть · переформулировать · проверить по компоненту, если противоречит.
3. В отчёте укажи, что перенесено и что НЕ перенесено — с причиной по каждому пункту.

Не переносить можно только это:
- размеры в px, длительности в ms и прочие параметры реализации → материал для `DEV NOTES`, но НЕ молча в никуда;
- упоминания прототипа Figma;
- утверждения, которые прямо противоречат компоненту (тогда верь компоненту и **скажи об этом в отчёте**).

Всё остальное — правила письма, лимиты, сценарии, запреты, рекомендации по композиции — переносится. Если правило нельзя проиллюстрировать превью, сохрани его текстом: потерять правило хуже, чем оставить его без картинки.

⚠️ Отдельно: если у дизайнера на странице лежат наработки помимо доки (например `*-copywriting`), они тоже источник и тоже переносятся.
...
## Per-variant coverage — types, defaults, states

When a component ships several **types/variants** in one property (напр. `type` у wheel-picker), документируй **каждый вариант отдельно** — своя карточка/подсекция с живым визуалом и своим `type`-кодом. Не сваливай несколько типов в одну карточку: если у карточки в коде-строке два+ типа (`birthday · date:date-hh-mm`) — это кандидат на разбиение.

- **Дефолтное состояние — обязательно назвать словами И показать.** Где у типа есть значение по умолчанию (пробег и год у wheel-picker — «—», пустое), карточка прямо говорит «По умолчанию — «—»» и показывает именно дефолт-колесо, а не заполненный пример.
- **Шаги/прогрессию показывай визуально, а не прячь в текстовую пилюлю.** Если значение меняется нелинейно (шаг пробега растёт: 1 000 → 5 000 → … → крупнее), ставь ряд колёс «дефолт → шаг → шаг» с точными подписями под каждым. Пользователь должен видеть механику, не вычитывать её.
- **Измеряй, не угадывай:** подписи под визуалом должны совпадать с тем, что реально показывает инстанс (свериться скриншотом), а не с текстом старой доки — старая дока может расходиться с текущим компонентом.
- **Зона нажатия (touch-area).** Смотри слои `*-touch-area` в компоненте/иллюстрациях: одна колонка (`values-touch-area` на всю ширину) → вся область = зона скролла и нажатия; несколько колонок → у каждой своя зона (`hours/minutes/from/to-touch-area`, разной ширины), соседи жест не перехватывают, минимум колонки 44 × 44 px.

**Материал для повариантных визуалов** часто уже есть в старой доке как чистые инстансы компонента (напр. колёса 393×200) — клонируй их в натуральном размере. Осторожно: `layoutMode: NONE`-слот (`screen`) НЕ переставляет вставленного ребёнка — после `appendChild` ставь `x=0; y=0`, иначе клон окажется по старым координатам и клипнется в белое. Растягивание старого инстанса на FILL ломает его внутреннюю раскладку — держи натуральный размер.
